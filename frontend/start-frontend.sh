...
sed \
  -e "s/__PORT__/${PORT}/g" \
  -e "s/__BACKEND_HOST__/${FULL_BACKEND_HOST}/g" \
  -e "s/__BACKEND_PORT__/${BACKEND_PORT}/g" \
  -e "s/__DNS_RESOLVER__/${DNS_RESOLVER}/g" \
  /etc/nginx/templates/default.conf.template > /etc/nginx/conf.d/default.conf

echo "[start-frontend] starting with BACKEND_HOST=${FULL_BACKEND_HOST}, BACKEND_PORT=${BACKEND_PORT}, PORT=${PORT}, RESOLVER=${DNS_RESOLVER}"
